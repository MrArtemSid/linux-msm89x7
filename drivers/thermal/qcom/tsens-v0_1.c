...
static int calibrate_8937_8917(struct tsens_priv *priv)
{
	int base0 = 0, base1 = 0, i;
	u32 p1[10], p2[10];
	int mode = 0;
	...
	case TWO_PT_CALIB:
		...
		if(priv->num_sensors == 11)
			p1[10] = (cdata[4] & MSM8937_S10_P2_MASK) >> MSM8937_S10_P2_SHIFT;
		for (i = 0; i < priv->num_sensors; i++)
			p2[i] = (base1 + p2[i]) << 2;
		fallthrough;
	default:
...
struct tsens_plat_data data_8917 = {
	.num_sensors	= 10,
	.ops		= &ops_8937,
